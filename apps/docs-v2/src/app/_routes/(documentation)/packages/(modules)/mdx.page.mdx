...
<StepItem
  ...
  content={`
    Use the \`Markdown\` component from \`@rasenganjs/mdx\` to render markdown content from a string. For example:
  `}
>
  ...
  layout: ({ children, toc }) => (
    <article className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex gap-8">
        ...
        {toc && (
          <aside className="hidden lg:block w-64 flex-shrink-0">{toc}</aside>
        )}
      </div>
    </article>
  ),
...
<Alert type="warning">This is an important warning message!</Alert>
