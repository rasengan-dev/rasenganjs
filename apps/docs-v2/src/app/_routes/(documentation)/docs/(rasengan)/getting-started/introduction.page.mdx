...
| **`API Routes`**   | Build backend endpoints alongside your pages with file-based `_api/` routes, no separate server required. |
| **`Backend Framework`** | Need a full backend? [`@rasenganjs/server`](/docs/server/getting-started/introduction) brings **controllers, dependency injection, and modules** on top of the same ecosystem. |
| **`Environment Variables`** | [Automatic `.env*` loading](/docs/configuring/environment-variables) into `process.env`, in both dev and build, with **zero `dotenv` setup**. |
| **`Deployment`**   | Ship to **Vercel, Netlify**, or self-host with `@rasenganjs/serve`, built-in adapters for every target. |
| **`WinterCG Compatible`** | Built on [Futon](/docs/futon/getting-started/introduction), a runtime-agnostic HTTP engine that runs the same code on **Node.js and Bun**. |
