...
<br />
Rasengan.js allows you to use SSR with the `loader` function inside your page
component. This function runs on the `server` and fetches data before rendering
the page.
...
const Profile: PageComponent<{ user: { name: string; email: string } }> = ({
  user,
}) => {
  return (
    <div>
