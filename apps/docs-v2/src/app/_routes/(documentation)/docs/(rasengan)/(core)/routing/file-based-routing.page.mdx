...
<Note title="Warning" type="warning">
  `app.router.ts` is a required file, you have to place it at the src/app/
  folder.
</Note>
...
<Note type="info" title="Note: Page Component">
  Don't define `path` manually onto the page definition, because it's directly
  extracted from the file and folder structure.
</Note>
...
<Note type="info" title="Note: Path Definition">
  When using `file based routing`, you don't need to define `path` manually onto
  the page and layout definition, because it's directly extracted from the file
