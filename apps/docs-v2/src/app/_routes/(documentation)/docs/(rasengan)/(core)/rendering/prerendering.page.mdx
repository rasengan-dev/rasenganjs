...
Instead of rendering pages in the browser (SPA) or on the server at request time (SSR), Rasengan creates static HTML files that can be deployed anywhere: a CDN, file storage, or any static hosting provider.
...
<Note type="info" title="Note">
  When the `prerender` is enabled, the production output is located into the
  `static` folder, instead of the `dist` one
</Note>
...
// src/app/_routes/blog.page.jsx => blog/index.html
