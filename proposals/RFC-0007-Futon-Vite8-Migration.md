...
- **✅ Done (2026-08-06), fast-follow picked up.** `generateServerlessHandler()`'s template rewritten on Futon + `@rasenganjs/runtime`'s Node adapter building blocks. Since the generated function only ever gets written when `config.ssr && !config.prerender` (`prepare()`'s own gate — SPA/SSG deploys are pure static hosting through `.vercel/output/static`, no function involved), the template only needs to cover the SSR case, unlike `@rasenganjs/serve`'s cli.ts which also has to handle SPA fallback and redirects (already handled here at the Vercel routing layer via `generateVercelConfigFile`'s `routes`, so not duplicated inside the function).
  - Confirms the "closer to `NodeProdAdapter`'s one-shot model" guess above: doesn't call `NodeProdAdapter.serve()` (which binds its own port — wrong for a serverless invocation), just reuses its filesystem-backed, traversal-protected `Assets` implementation via `new NodeProdAdapter({ rootDir: process.cwd() }).assets`, then `app.configureAssets(...)` manually. Exports a plain `async (req, res) => {...}` — the same calling convention an Express app satisfies (`incomingToRequest`/`writeNodeResponse`, both from `@rasenganjs/runtime/adapters/node`, do the Node↔Web `Request`/`Response` bridging), so it drops into Vercel's Node.js launcher with no Vercel-specific glue needed.
  - `app.init()` is called once at module scope (fired on cold start, awaited-but-already-resolved on warm invocations) rather than per-request, matching how `NodeProdAdapter.serve()` calls it once before listening.
  - Two of the original handler's three `express.static(...)` mounts carried over as `staticFiles()` (client `/assets`, immutable/1y; client root, 1h) — mostly a safety net since Vercel's `{handle: 'filesystem'}` routing already serves everything under `.vercel/output/static` directly, before the catch-all route ever reaches the function. The third mount, `express.static('public', ...)`, was dropped: `prepare()` never copies a `public/` directory into the function bundle (only `dist/server` → `server/` and `dist/client` → `client/`), so that mount only ever 404'd through to the SSR handler on real Vercel deploys — dead code, not a behavior to preserve.
  - Also added `createMatchRoutesGuard` ahead of `createRequestHandler` in the generated `app.fallback(...)`, matching what `@rasenganjs/serve` already does for the identical production SSR path (RFC-0007 §3) — not present in the original handler, which predates that guard.
  - **Verified end-to-end**, not just "it compiles": built `@rasenganjs/vercel`, then ran a real `rasengan build` with `VERCEL=1` against `apps/playground/file-based-routing` temporarily configured for `ssr: true` + the Vercel adapter. Confirmed `.vercel/output/functions/index.func/{index.js,package.json,.vc-config.json,server/,client/}` all generated correctly, then invoked the generated `index.js`'s default export directly against a real `http.Server` (symlinking `@rasenganjs/futon`/`@rasenganjs/runtime` into the function's `node_modules` to resolve the same way a real `npm install` would): `GET /` → 200 with correct SSR HTML (`<div id="root">`, correct `<title>`), `GET /assets/*.css` → 200 with byte-identical content. Cross-checked `GET /does-not-exist-at-all` → 200 (not 404) against `@rasenganjs/serve` on the _same build_ and got the identical result — confirms this is a pre-existing, framework-wide gap (the catch-all route's loader returns a plain object instead of a `Response` with a 404 status, so `createRequestHandler` never sets one), not something this phase introduced or could fix by itself.

**Phase 2d — Catch-all route never actually returns HTTP 404 (found during Phase 2c verification, 2026-08-06)**

- **Found:** the generated `path: '*'` catch-all route's `loader` (`routing/utils/generate-routes.tsx`) returned a plain `{ props, meta }` object. React Router's `createStaticHandler().query()` only elevates `context.statusCode` above 200 when a loader's result carries its own status — a plain object never does, so every genuinely-unmatched URL rendered the "Not Found" UI while responding `200`. Confirmed identical on `main` via `@rasenganjs/serve` on the same build — not a regression from this branch, a pre-existing gap.
- **Fix:** the catch-all loader now returns `data({ props, meta }, { status: 404 })` (react-router's `data()` helper) instead of a bare object — `processRouteLoaderData` picks up `result.statusCode` from `data()`-wrapped results and propagates it to `context.statusCode`, which `createRequestHandler` already threaded into `render()`'s `statusCode` option (no change needed there).
- **Second, deeper bug this uncovered:** `handler.queryRoute()` (used by `handleDataRequest` for `.data`/`Accept: application/json` requests) doesn't _return_ a `data()`-wrapped result the way `handler.query()` does — per its own documented contract, it _throws_ the constructed `Response` for any loader result carrying its own status/headers (redirects, thrown `Response`s, and now `data()` with a non-default init), success or error alike, so the caller can distinguish "here's your final Response" from "call this in a request/response context." `handleDataRequest` had no `try/catch` around `handler.queryRoute(...)`, so a `.data` request to an unmatched path threw all the way out to `createRequestHandler`'s outer catch and came back as a generic `500`, not a `404`. Fixed by catching and returning the thrown `Response` directly when `result instanceof Response`; anything else still rethrows to preserve existing error behavior. This second bug is not new either — any custom loader that has always been free to `throw new Response(...)` for a redirect/error would have hit the exact same gap; the catch-all's own loader is just the first place in rasengan's own code to exercise it.
- **Verified:** `apps/playground/file-based-routing`, both SSR and SSG (prerender) modes. Document requests: real pages `200`, a genuinely unmatched multi-segment path `404` (note: single-segment "unmatched" paths like `/does-not-exist` are _not_ a valid test here — they match the app's own `[_locale]` optional dynamic segment as a bogus locale value and resolve to the home page, which is correct routing behavior, not a bug). `.data` requests: real pages still `200` with correct JSON, the same unmatched path now `404` with a valid `{"props":{},"meta":{...}}` body instead of a `500`. SSG prerendering (`static/404.html`) unaffected — still generates cleanly.
