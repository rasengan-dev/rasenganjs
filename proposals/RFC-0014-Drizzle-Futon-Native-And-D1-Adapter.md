...
**Consequence for this RFC:** `createDrizzlePlugin()` as sketched above is correct in shape, matches the same pattern `@rasenganjs/ws` already uses, but its `register()` callback's `app.use()` call is a no-op against the current `rasengan-server`. The D1-via-`forRoot()` path (and any future resolver-based adapter used through the DI path) cannot ship until this is fixed upstream. **Fixing it is out of scope for this RFC**, the same way RFC-0006 scoped the `useValue` fix to what became RFC-0012, it is a `rasengan-server` core issue, not a `@rasenganjs/drizzle` one. Two candidate fixes for that follow-up RFC, noted here so the actual fix has a starting point:

- Move the `middlewareList` drain (step 1) to run after `dispatchPlugins()` (step 3) instead of before it. Needs a careful check of whatever else in `compile()` currently assumes global middleware is attached before routes are registered, this is a real behavior change to `compile()`'s sequencing, not a one-line move.
- Give `ModulePlugin.register()` a dedicated way to add middleware that writes directly onto `this.futon` (already assigned at line 343, before `dispatchPlugins()` runs), instead of going through the `ServerApp.use()`/`middlewareList` path built for pre-`compile()` calls from `bootstrap()`. Smaller, more localized change, but `this.futon` is currently private with no plugin-facing accessor, so it still needs a new, deliberate `ServerApp` API, not just exposing the field.

Everything else in this RFC (`connection.ts`, `futon.ts`, `drivers/d1.ts`) has no dependency on this constraint and is fully buildable and shippable on its own. Only the DI path's lazy-source case depends on it, `DrizzleModule.forRoot()` with a static/pg config touches no plugin at all and is equally unaffected.
...
1. **`connection.ts` core**: `createConnection()`, the eager path, the lazy path, the concurrent-first-request guard, `onClose()`/`close()` wiring. Pure unit tests, no Futon or DI dependency, mirroring how RFC-0006's own `drizzle.module.test.ts` used a fake adapter. No dependency on the `rasengan-server` bug, ships first.
2. **`futon.ts` and `drivers/d1.ts`**: the Futon-native `drizzle()` helper built on the core, `d1Adapter()`, tests against a real (not mocked) `Futon` instance to exercise `app.use()`/`app.onDestroy()` for real. Fully usable end to end at this point, including D1 on Futon directly, no `rasengan-server` involved.
3. **`drizzle.module.ts` and `data-source.ts` rewritten on the core, static/pg case only**: `DataSource.db`/`onDestroy()` delegate to `connection.ts`, the two distinct "not ready" error messages. Every existing RFC-0006 test for the `node-postgres`/eager case must keep passing unchanged. No `ModulePlugin` involved yet, unaffected by the core bug.
4. **`createDrizzlePlugin()` and the lazy/D1 path through `forRoot()`**: blocked until the `rasengan-server` core fix (see "Dependency" section) lands. Can be developed and tested against a local patched build of `rasengan-server` in the interim, not published as working until the upstream fix ships.
5. **Docs and examples**: a Futon-only Postgres example and a Futon-only D1 example (both unblocked after Phase 2) targeting Cloudflare Workers, plus a Rasengan-Server D1 example once Phase 4 unblocks, both under `apps/playground` or `apps/examples`, matching RFC-0006's own example precedent.
6. **Publish**: joins the existing `@rasenganjs/drizzle` release cycle. The Futon-native path and the `node-postgres`/DI path can publish independently of Phase 4 if the core fix is still pending, the D1-via-`forRoot()` surface stays undocumented/unpublished until then.

## Verification (for the eventual implementation PR)

- `connection.ts`: eager source connects immediately at creation, lazy source connects only after the first `resolve()` call, a second concurrent `resolve()` call while the first is in flight does not call `adapter.connect()` a second time, `close()` calls the adapter's `close()` exactly once plus every registered close hook.
- `futon.ts`: `getDb()` throws before the first request for a lazy source, works immediately for a static source, works after the first request for a lazy source and stays stable across further requests, `app.onDestroy()` closes the underlying connection.
- `drivers/d1.ts`: `connect()` wraps `drizzle-orm/d1` correctly against a fake `D1Database`, `migrate()` throws the documented pointer to `wrangler d1 migrations apply`.
- `drizzle.module.ts`: every existing RFC-0006 test (double `forRoot()` throws, DI-by-name contract, `.db` delegation, `onDestroy()` closes) still passes unchanged for the `node-postgres`/eager case.
- `createDrizzlePlugin()`, once the `rasengan-server` fix lands: an integration test that a plugin-registered middleware actually runs before a matched controller route handler, not only before the 404 fallback, this is the exact property the current bug breaks. Plus `DataSource.db` throwing the distinct "waiting for first request" message before any request, and returning the connected database after one.
- Workspace typecheck clean, full vitest suite green.
