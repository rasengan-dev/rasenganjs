...
describe('BunAssets', () => {
  ...
  before(() => {
    tmp = mkdtempSync(join(tmpdir(), 'rg-bun-assets-'));
    assets = new BunAssets(tmp);
  });
  ...
  it('load returns text content', async () => {
    const data = new TextEncoder().encode('hello text');
    await assets.write('greeting.txt', data);
    const content = await assets.load('greeting.txt');
...
describe('BunWatcher', () => {
  it('watch fires callback on file change and dispose stops it', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'rg-bun-watch-'));
    ...
    const dispose = watcher.watch(testFile, () => {
      callCount++;
    });
...
describe('BunDevAdapter', () => {
  ...
  before(async () => {
    ...
    adapter = new BunDevAdapter({ port: PORT });
    adapter.serve(app);
    started = true;
    await new Promise((r) => setTimeout(r, 200));
  });
...
describe('BunProdAdapter', () => {
  ...
  before(async () => {
    ...
    adapter = new BunProdAdapter({ port: PORT });
    adapter.serve(app);
    started = true;
    await new Promise((r) => setTimeout(r, 200));
  });
...
function entryScript(version) {
  return [
    ...
    `Bun.serve({ fetch: (request) => app.fetch(request), port });`,
  ].join('\n');
}

describe('BunDevAdapter auto-restart', () => {
  let adapter;
  let started;
  const PORT = 15342;
