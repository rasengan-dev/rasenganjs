 ...
 *
 * Also assigns every merged key into `process.env`, skipping any key
 * already set there (RFC-0010) — a real environment variable (shell,
 * CI, a platform-injected secret) always wins over a `.env*` file's
 * value. This is what makes the standard `process.env.X` idiom work
 * for app code and third-party libraries alike; the returned map is
 * still fed into `Futon.loadEnv()` by callers for the portable
 ...
 * in the same priority order `loadNodeEnvFiles` reads them (RFC-0010).
 *
 * Display-only — a separate existence check rather than a return value
 * of `loadNodeEnvFiles` itself, so callers that only need the merged
 * map (the vast majority) aren't forced to destructure a wrapper
 * object. Startup-only cost (at most 4 stat calls).
 *
 * @param rootDir - Project root directory containing .env files.
 * @param mode    - Current environment (`development` or `production`).
