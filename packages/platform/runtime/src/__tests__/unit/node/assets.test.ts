...
import { NodeAssets } from '../../../adapters/node/assets.js';

describe('NodeAssets', () => {
  let rootDir: string;
  let assets: NodeAssets;

  beforeEach(async () => {
    rootDir = join(tmpdir(), `rasengan-assets-test-${randomUUID()}`);
    await mkdir(rootDir, { recursive: true });
    assets = new NodeAssets(rootDir);
  });
  ...
  describe('get', () => {
    ...
    it('returns null for a directory', async () => {
      ...
      const result = await assets.get('subdir');
      expect(result).toBeNull();
    });
  });
  ...
  describe('path traversal protection', () => {
    ...
    it('throws with path traversal using ../', async () => {
      await expect(assets.get('../../../etc/passwd')).rejects.toThrow(
        'Path traversal detected'
      );
    });
